...
// zod 定義的內容庫 schema。目前只負責定義,還沒有任何地方 import 這支檔案、
// 也還沒接進 build.js 的 parsing 流程——純粹先把討論定案的形狀寫下來。
//
// 命名慣例:XxxSchema 是 zod 物件本身;解析出的原始型別直接用 Xxx(跟 schema
// 同一個詞根,不加字首);fk 解析成完整關聯物件之後的型別才加 Resolved 字首
// (比如 ResolvedTranslation),因為那已經不是 zod.parse() 的直接產物,是
// build.js 在 resolveAll() 階段手動 & 擴增出來的。
//
// date 欄位一律是 z.string()(YYYY-MM-DD),不是 z.date()——gray-matter 底層
// js-yaml 預設會把這種字串自動轉成 Date 物件,若欄位型別也定義成 Date,容易
// 讓後面的程式碼直接對 Date 用 local timezone 相關方法(toLocaleDateString、
// 樣板字串內插等),不同時區下會讀出差一天的日期。要根治要在 gray-matter 的
// yaml engine 選項換成 yaml.JSON_SCHEMA(不含 !!timestamp 型別),讓日期从頭
// 到尾都只是字串;這裡的 schema 先假設呼叫端已經拿到字串,不在這支檔案處理。
// ISO 8601 字串本身字典序等於時間序,排序不需要轉回 Date。
...
const FirstPublishedSchema = z.object({
  title: z.string(),
  publisher: z.string(),
  date: z.string(),
});
...
const SourceTranslatorSchema = z.object({
  uuid: z.string().uuid(),
  names: LocalizedStringSchema,
  language: z.enum(SUPPORTED_LANGUAGES),
  wikidata_id: z.string().optional(),
